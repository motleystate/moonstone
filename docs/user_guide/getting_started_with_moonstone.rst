...
Then, you might need to filter the data of poor quality or with missing metadata from your pandas dataframe.
...
To run several different filtering methods on your data, the filtering must be run one after the other. The filtered dataframe generated by the previous filtering needs to be given as input to the next filtering.
